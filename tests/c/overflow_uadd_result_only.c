...
//     %{{_}}: i32 = call %{{h_fprintf}}(%{{h_stream}}, %{{h_fmt}}, %{{h_i}}, %{{h_uadd_r}}) ; @fprintf
//     ...
//     --- End hir ---
//     4: uadd=4294967295
...
// Check that llvm.uadd.with.overflow behaves correctly when the overflow flag is unused.
...
int main(int argc, char **argv) {
  ...
  while (i > 0) {
    ...
    unsigned int result;
    NOOPT_VAL(ua);
    __builtin_uadd_overflow(ua, 2, &result);

    fprintf(stderr, "%d: uadd=%u\n", i, result);
    i--;
  }
  fprintf(stderr, "exit\n");
