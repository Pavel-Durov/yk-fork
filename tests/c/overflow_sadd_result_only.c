...
//     %{{_}}: i32 = call %{{h_fprintf}}(%{{h_stream}}, %{{h_fmt}}, %{{h_i}}, %{{h_sadd_r}}) ; @fprintf
//     ...
//     --- End hir ---
//     4: sadd=2147483647
...
// Check that llvm.sadd.with.overflow behaves correctly when the overflow flag is unused.
...
int main(int argc, char **argv) {
  ...
  while (i > 0) {
    ...
    int result;
    NOOPT_VAL(sa);
    __builtin_sadd_overflow(sa, 2, &result);

    fprintf(stderr, "%d: sadd=%d\n", i, result);
    i--;
  }
  fprintf(stderr, "exit\n");
