...
//     %{{_}}: i32 = call %{{h_fprintf}}(%{{h_stream}}, %{{h_fmt}}, %{{h_i}}, %{{h_usub_r}}) ; @fprintf
//     ...
//     --- End hir ---
//     4: usub=3
...
// Check that llvm.usub.with.overflow behaves correctly when the overflow flag is unused.
...
int main(int argc, char **argv) {
  ...
  while (i > 0) {
    ...
    unsigned int result;
    NOOPT_VAL(ua);
    __builtin_usub_overflow(ua, 2, &result);

    fprintf(stderr, "%d: usub=%u\n", i, result);
    i--;
  }
  fprintf(stderr, "exit\n");
