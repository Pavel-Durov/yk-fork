...
impl<'a> X64HirToAsm<'a> {
    ...
    fn i_overflow(
        ...
        // both is emitted here rather than at the sadd_overflow/uadd_overflow/usub_overflow
        // instruction, which has no single register that could hold both results. Codegen is
        // emitted at the flag extractval, so the result extractval early-returns if it sees the
        // flag is also used.
...
impl HirToAsmBackend for X64HirToAsm<'_> {
    ...
    fn i_extractval(
        ...
        match val_inst {
            ...
            _ => panic!(
                "extractval operand is not a call, sadd_overflow, uadd_overflow or usub_overflow"
            ),
        }
    }
