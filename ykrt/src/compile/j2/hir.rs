            ...
            for op_iidx in inst.iter_iidxs(self) {
                    ...
                    self.inst(op_iidx)
                {
                    match inst {
                        Inst::ExtractVal(_) => (),
                        _ => panic!(
                            "%{iidx:?}: sadd_overflow/uadd_overflow/usub_overflow result must only be accessed via extractval"
                        ),
                    }
                }
            }
...
impl InstT for ExtractVal {
    fn assert_well_formed(&self, m: &dyn ModLikeT, b: &dyn BlockLikeT, iidx: InstIdx) {
        assert_matches!(
            ...
            "%{iidx:?}: extractval operand is not a call, sadd_overflow, uadd_overflow or usub_overflow"
        );
        assert!(
...
mod test {
    ...
    #[test]
    #[should_panic(
        expected = "%2: sadd_overflow/uadd_overflow/usub_overflow result must only be accessed via extractval"
    )]
