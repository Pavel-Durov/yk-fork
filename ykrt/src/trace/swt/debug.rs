...
pub(crate) fn get_rbp_value_at_offset(rbp_offset: i32, src_val_size: &u16) -> u64 {
    ...
    for (_, src_var) in src_rec.live_vals.iter().enumerate() {
        ...
        match location {
            Indirect(_, src_off, src_val_size) => {
                let value = get_rbp_value_at_offset(*src_off, src_val_size);
            ...
            Register(src_reg_num, src_val_size, _src_add_locs) => {
                ...
                let value = get_rbp_value_at_offset(-reg_store_rbp_offset, src_val_size);
                println!("{:?}\tvalue=0x{:016x}", location, value);
            }
    ...
    for (_, dst_var) in dst_rec.live_vals.iter().enumerate() {
        ...
        match location {
            Indirect(_, dst_off, dst_val_size) => {
                let value = get_rbp_value_at_offset(*dst_off, dst_val_size);
                println!("{:?}\tvalue=0x{:016x}", location, value);
            }
            Register(dst_reg_num, dst_val_size, dst_add_locs) => {
                ...
                for add_loc in dst_add_locs {
                    if *add_loc >= 0 {
                        ...
                        let value = get_rbp_value_at_offset(rbp_offset, dst_val_size);
                        println!(
                ...
                if dst_add_locs.len() == 0 {
                    unsafe {
                        let value = get_reg_value(*dst_reg_num);
                        println!("{:?}\tvalue=0x{:016x} (may be clobbered)", location, value);
                    }
                }
            }
            Direct(_, src_off, src_val_size) => {
